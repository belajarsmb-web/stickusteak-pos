<?php
/**
 * RestoQwen POS - Kitchen Orders API
 * GET /api/kds/kitchen-orders.php - Get food orders for kitchen
 */
...
try {
    ...
    // Get orders with food/kitchen items
    // Include all active kitchen statuses but exclude served
    $sql = "SELECT DISTINCT o.id, o.table_id, o.status, o.created_at,
            ...
            AND (m.display_routing IN ('kitchen', 'both') OR m.display_routing IS NULL)
            AND oi.is_voided = 0
    ...
    // Calculate cooking time and priority
    foreach ($orders as &$order) {
        $created = new DateTime($order['created_at']);
        $now = new DateTime();
        $interval = $created->diff($now);
        
        $order['cooking_time_minutes'] = $interval->h * 60 + $interval->i;
        $order['cooking_time_formatted'] = sprintf('%dh %dm', $interval->h, $interval->i);
        
        // Priority based on wait time
        if ($order['cooking_time_minutes'] > 30) {
            $order['priority'] = 'high';
        } elseif ($order['cooking_time_minutes'] > 15) {
