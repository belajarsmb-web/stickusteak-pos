<?php
/**
 * Stickusteak POS - Kitchen Display System (KDS)
...
<body>
    ...
    <!-- KDS Container -->
    <div class="kds-container" id="kdsContainer">
    ...
    <audio id="newOrderSound" preload="auto">
        <source src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbqWEyMmih0NupYTIyaKHQ26lhMjJoodDbqWEyMmih0NupYTIyaKHQ26lhMjJoodDbqWEyMmih0NupYTIy" type="audio/wav">
    </audio>
    ...
    <script>
        ...
        async function loadOrders() {
            try {
                const response = await fetch('/php-native/api/kds/kitchen-orders.php?t=' + Date.now());
        ...
        function renderOrders(orders) {
            ...
            orders.forEach(order => {
                ...
                const timerColor = order.cooking_time_minutes > 30 ? '#dc3545' :
                                   order.cooking_time_minutes > 15 ? '#ffc107' : '#28a745';
                    ...
                    <div class="order-card ${priorityClass}">
                        <div class="order-header">
                            ...
                            <div class="order-timer" style="border-color: ${timerColor}; color: ${timerColor};">
                                ⏱️ ${order.cooking_time_formatted}
                            </div>
                        </div>
                        ...
                        <div class="order-footer">
                            ${order.status === 'sent_to_kitchen' ? `
                                <button class="btn-kds btn-start" onclick="updateOrderStatus(${order.id}, 'in_progress')">
                                    🔥 Start Cooking
                                </button>
                            ` : ''}
                            ${order.status === 'in_progress' ? `
                                <button class="btn-kds btn-ready" onclick="updateOrderStatus(${order.id}, 'ready')">
                                    ✅ Ready to Serve
                                </button>
                            ` : ''}
                            ${order.status === 'ready' ? `
                                <button class="btn-kds btn-served" onclick="updateOrderStatus(${order.id}, 'served')">
                                    🎉 Mark Served
                                </button>
                            ` : ''}
                            ${order.status === 'served' ? `
        ...
        function updateStats(orders) {
            ...
            const avgTime = orders.length > 0 
                ? Math.round(orders.reduce((sum, o) => sum + o.cooking_time_minutes, 0) / orders.length)
                : 0;
        ...
        function playNotificationSound() {
            ...
            setTimeout(() => {
                document.title = '👨‍🍳 Kitchen Display - KDS';
            }, 5000);
        }
